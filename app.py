...
def clean_title(title):
    ...
    return re.sub(r'\s+', ' ', title).strip(' .')[:180] or 'video'
...
def get_settings():
    ...
    r['concurrent_downloads'] = int(r.get('concurrent_downloads', 2))
    r['speed_limit'] = int(r.get('speed_limit', 0))
...
def download_options(job_id, url, quality, media_type):
    ...
    outtmpl = os.path.join(folder, f'{job_id}.%(ext)s')
    ...
    def hook(d):
        ...
        if d.get('status') == 'downloading':
            ...
            percent = round(min(99, done * 100 / total), 1) if total else 0
...
def run_download(job_id, url, quality, media_type='video'):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            c = db()
            c.execute('INSERT INTO history(title,url,filename,path,status,media_type,quality,size,created_at) VALUES(?,?,?,?,?,?,?,?,?)', (title, url, os.path.basename(destination), destination, 'completed', media_type, quality, size, time.time()))
            c.commit()
            c.close()
...
def api_progress(job_id):
    ...
    if job.get('status') == 'downloading' and time.time() - job.get('last_progress_at', job.get('updated_at', time.time())) > STALE_SECONDS:
...
def api_retry(job_id):
    ...
    JOBS[new] = {
        ...
        'media_type': j.get('media_type', 'video'),
        'status': 'queued',
        'percent': 0,
        'message': 'در صف دانلود...',
        'created_at': time.time()
    }
    CANCEL_EVENTS[new] = threading.Event()
    EXECUTOR.submit(run_download, new, j['url'], j.get('quality', 'best'), j.get('media_type', 'video'))
...
def api_file(job_id):
    ...
    if not p and str(job_id).isdigit():
        c = db()
        row = c.execute('SELECT path,filename FROM history WHERE id=?', (int(job_id),)).fetchone()
        c.close()
        if row:
            p, filename = row['path'], row['filename']
...
def api_delete_history(item_id):
    c = db()
    row = c.execute('SELECT path FROM history WHERE id=?', (item_id,)).fetchone()
...
def api_put_settings():
    ...
    if 'concurrent_downloads' in d:
        try:
            d['concurrent_downloads'] = max(1, min(5, int(d['concurrent_downloads'])))
        except (TypeError, ValueError):
            d.pop('concurrent_downloads', None)
    if 'speed_limit' in d:
        try:
            d['speed_limit'] = max(0, int(d['speed_limit']))
        except (TypeError, ValueError):
...
def api_presets():
    c = db()
    rows = c.execute('SELECT * FROM presets ORDER BY created_at DESC').fetchall()
    c.close()
...
def api_create_preset():
    d = request.get_json(silent=True) or {}
    name = str(d.get('name', 'Preset')).strip() or 'Preset'
    c = db()
    cur = c.execute('INSERT INTO presets(name,settings,created_at) VALUES(?,?,?)', (name, json.dumps(d.get('settings') or {}, ensure_ascii=False), time.time()))
    c.commit()
    pid = cur.lastrowid
    c.close()
...
def font(name):
    return send_from_directory(os.path.join(STATIC_FOLDER, 'font'), name)
